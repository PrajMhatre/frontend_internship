...
function Login() {
  ...
  // Log login activity in Firestore
  const logLoginActivity = async (user) => {
    try {
      const loginDocRef = doc(db, 'loginHistory', user.uid);
      await setDoc(loginDocRef, {
        lastLogin: Timestamp.now(),
        email: user.email,
      });
      console.log('Login activity logged successfully:', user.uid);
    } catch (error) {
              ...
              <div className="flex bg-white rounded-lg justify-center overflow-hidden mx-auto max-w-sm lg:max-w-4xl">
                <div className="w-full p-8 lg:w-1/2">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <p className="text-gray-500 text sm font-bold">{t('register.or')}</p>
                    <span className="border-b- w-1/5 lg:w-1/4"></span>
                  </div>
